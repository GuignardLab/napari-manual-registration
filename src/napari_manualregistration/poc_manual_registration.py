...
class RegistrationWidget(Container):
    def __init__(self, viewer: "napari.viewer.Viewer"):
        ...
        self._translate_z.changed.connect(self._reset_transfos_if_layers_dont_exist)
        ...
        self._translate_y.changed.connect(self._reset_transfos_if_layers_dont_exist)
        ...
        self._translate_x.changed.connect(self._reset_transfos_if_layers_dont_exist)

        #rotations : moving the layer
        # self._rotate_z = create_widget(
        #     widget_type="IntSlider", label="rz",
        #     options={'min':-180, 'max':180}
        # )
        # self._rotate_z.changed.connect(self._update_rotation)
        # self._rotate_z.changed.connect(self._reset_transfos_if_layers_dont_exist)
        ...
        # self._rotate_y.changed.connect(self._reset_transfos_if_layers_dont_exist)
        ...
        # self._rotate_x.changed.connect(self._reset_transfos_if_layers_dont_exist)
        ...
        self._test_slider_rz.changed.connect(self._update_rotation)
        self._test_slider_rz.changed.connect(self._reset_transfos_if_layers_dont_exist)
        ...
        self._test_slider_ry.changed.connect(self._reset_transfos_if_layers_dont_exist)
        ...
        self._test_slider_rx.changed.connect(self._reset_transfos_if_layers_dont_exist)
        ...
        self._save_json_path = create_widget(
            ...
            options={'mode':'d'},
            label='Save the transformations :'
        )

        self._save_json_button = create_widget(
            widget_type="PushButton", label='Save into JSON'
        )

        self._save_json_button.clicked.connect(self._save_into_json)
    ...
    # def _choose_directory_callback(self):
    ...
    #         # self._directory_line_edit.setText(directory)
    ...
    def _reset_transfos_if_layers_dont_exist(self,event):
        if self._layer_floating.value is None:
            ...
            self._test_slider_rz.value = 0
            self._test_slider_ry.value = 0
            self._test_slider_rx.value = 0
            self._translate_z.value = 0
            self._translate_y.value = 0
            self._translate_x.value = 0

    def _save_into_json(self):
        ...
        if path == '.' or not os.path.exists(path):
            ...
            data_to_save = {
                "rot_z": self._test_slider_rz.value,
                "rot_y": self._test_slider_ry.value,
                "rot_x": self._test_slider_rx.value,
    ...
    def _update_sliders(self, translation_vector, rotation_matrix):
        ...
        self._test_slider_rz.value = angles[0]
        self._test_slider_ry.value = angles[1]
        self._test_slider_rx.value = angles[2]
